...
def process_file(filename, data_type, word_counter, char_counter):
    ...
    with open(filename, "r") as fh:
        source = json.load(fh)
        for article in tqdm(source["data"]):  ####  source["data"]    ####################################
            # 从第一个段落开始遍历 442个paragraphs
            for para in article["paragraphs"]:
                # 替换段落中的''和`` 19035个context 个数与qas相同
                ...
                for token in context_tokens:
                    word_counter[token] += len(para["qas"])    ##### 为什么加para["qas"]？？ 19035个qas #################
                    for char in token:
                        char_counter[char] += len(para["qas"]) ##### para["qas"]####################################
...
def pre_process(args):
    ...
    # glove_dim=300,  glove_num_vecs=2196017
